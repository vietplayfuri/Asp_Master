    ...
    $scope.submitTransferForm = function () {
        if ($scope.transferFormValid()) {
            ...
            $http({
                ...
                if ("correct" in data) {
                    var current = parseFloat($(".gtoken-number").text());
                    $(".gtoken-number").text(parseFloat(data["amount"]) + current);
            ...
            $http({
                ...
                if ("success" in data) {
                    if (data["is_free"] == true) {
                        var current = parseFloat($(".free-gtoken").text());
                        $(".free-gtoken").text(parseFloat(data["amount"]) + current);
                    } else {
                        var current = parseFloat($(".gtoken-number").text());
                        $(".gtoken-number").text(parseFloat(data["amount"]) + current);
                    }
...
function GameCtrl($scope, $http) {
    ...
    $scope.FirstLoadUI = function (pageIndex) {
        var urls = window.location.href.split(window.location.origin)[1].split('/');
        if (urls.length >= 6) {
            var params = new Object();
            params.keyword = urls[2] == "#all-keyword" ? "all-keyword" : urls[2];
            params.genre = urls[3];
            params.platform = urls[4];
            params.release = urls[5];
            params.pageIndex = urls[6];
            $scope.getGenres();
            searchGames(params);
            $scope.genre = urls[3];
            $scope.selectedGenre = urls[3];
        }
        else {
            ...
            $http({
                method: 'post',
                url: '/game/getgames/' + (pageIndex || 1),
                headers: {
                    'Content-type': 'application/json;'
                }
            }).success(function (data) {
                $scope.allGamesList = data.data.games;
                ...
                $scope.allGenres = data.data.genres;
    ...
    $scope.movetoPage = function (e, pageIndex) {
        e.preventDefault();
        if ($scope.pageNumber === pageIndex) return;
        $scope.pageNumber = pageIndex;
        $scope.FirstLoadUI(pageIndex);
    }
    ...
    $scope.searchGames_UI_change = function (pageIndex) {
        ...
        params.release = $scope.release;
        params.pageIndex = (pageIndex || 1);
        params.keyword = ($('input#game-input_value').val() || 'all-keyword');
    ...
    window.searchGames = $scope.searchGames = function (routeParams) {
        ...
        $http({
            ...
            $scope.totalItems = data.totalGames;
    ...
    $scope.genreClick = function (data) {
        router.navigate('/game/#' + ($('input#game-input_value').val() || 'all-keyword')
            + '/' + data.target.attributes["ng-data"].value
            + '/' + $scope.platform
            + '/' + $scope.release + '/1');

        var params = new Object();
        params.genre = data.target.attributes["ng-data"].value;
        params.platform = $scope.platform;
        params.release = $scope.release;
        params.pageIndex = 1;
        params.keyword = ($('input#game-input_value').val() || 'all-keyword');
