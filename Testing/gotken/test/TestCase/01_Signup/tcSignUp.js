...
casper.test.begin('\n\n TC02_Signup_Complete \n', function suite(test) {
    ...
    casper.thenOpen(urlRegister).waitForText(sTitleSignUpPage, function() {
        ...
        casper.test.fail('Cannot open Register Page!');
    });
...
casper.test.begin('\n\n TC03_Signup_Exist_Username_Email \n', function suite(test) {
    ...
    casper.start(urlRegister).waitForText(sTitleSignUpPage, function() {
...
casper.test.begin('\n\n TC07_Signup_Invalid_Email_With_Special_Char \n', function suite(test) {
    ...
    casper.start(urlRegister).waitForText(sTitleSignUpPage, function() {
        casper.each(lstSpecialChars, function(self, char) {
            this.reload(function() {
                casper.waitForText(sTitleSignUpPage, function() {
                    casper.wait(2000, function() {
...
casper.test.begin('\n\n TC10_Signup_With_ReferralID \n', function suite(test) {
    ...
    casper.thenClick(tabTransaction).waitForText(sTitleTransactionTab);

    // Verify Transaction History
    casper.then(function() {
        casper.acCheckTransactionHistory('small-gtoken', 'You received 5 Free GToken', 5);
    });
...
casper.test.begin('\n\n TC11_Signup_Add_ReferralID \n', function suite(test) {
    ...
    // Verify Transaction History
    casper.then(function() {
        casper.acCheckTransactionHistory('small-gtoken', 'You received 4 Free GToken', 4);
    });
